...
| # | Item | Evidence | Exit criterion |
| --- | --- | --- | --- |
| A1 | **`ScheduleData`** (`schedule_data.py`) -- import, export, save, load | The export the interface offers -- results as tab-separated text, plots as PNG -- lives inside a `QDialog` and a `QThread`. `p_dialog_export_calculated_data.py` is 312 lines of which only 60 touch Qt: **252 lines a CLI would have to reimplement and a server could not reach**. Save and load are already on `ScheduleProject` and already work without Qt, so the model keeps the code -- what is missing is the operation in front of it, because a method among operations is the odd one out: a CLI needs a special case, the journal records every calculation of a session but not the save that ended it, and a server needs an endpoint outside the request model | Every data operation is a request; the dialog keeps the file chooser and the progress bar; the model keeps its serialization and the `Super` is a facade over it, not a second implementation **Started, and the approach corrected before it did damage.** Lifting the 252 lines out by script -- dedent, rename `self.targets` to `targets`, turn `progress.emit` into a callback -- produced a file that would not parse, and a mechanical move of tangled code is exactly where a silent behaviour change hides. It is a hand move, method by method, with the verification already in place: an export of five calculations from the fixture, hashed per file (`OBS_DEFAULT_UV_Coverage.txt` and four others), so the moved code has to produce the same bytes. The seam is settled and is the useful part: `progress` and `cancelled` as plain callables, so the module knows nothing of signals, threads or windows |
