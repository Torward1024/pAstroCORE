...
| A3 | **The rest of the interface, tab by tab** | Counting lines in the hand-written GUI files that match no Qt name gives **5 861 of 7 772, about 75%** -- an *upper bound* on the gap rather than the amount of extractable logic, since much of it is glue that only means anything beside a widget. The one file read closely rather than counted was 252 of 312. Largest: `p_dialog_generate_observations.py` 361, `p_tab_telescopes.py` 344, `p_tab_scans.py` 318 | Each moves when something outside the GUI needs what it holds. Not one item, and not a sweep **First batch attempted and reverted.** The ten visualization tabs do share a question -- *which scans does this result cover for this source, and when does each start* -- and it is now one operation, `export(method="scan_times")`, with tests. What failed was the *edit*: a regex spanning from one method's first line to another method's loop deleted **851 lines** across four tabs, including whole methods, and the suite passed because nothing exercises them. Reported from the running application as `source_name` and `current_checks` undefined. Reverted. The operation stays; the tabs get changed by hand, one at a time, and the gap that let this through -- GUI tests that import modules but never call their methods -- is the real finding |
