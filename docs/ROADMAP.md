...
| A3 | **The rest of the interface, tab by tab** | Counting lines in the hand-written GUI files that match no Qt name gives **5 861 of 7 772, about 75%** -- an *upper bound* on the gap rather than the amount of extractable logic, since much of it is glue that only means anything beside a widget. The one file read closely rather than counted was 252 of 312. Largest: `p_dialog_generate_observations.py` 361, `p_tab_telescopes.py` 344, `p_tab_scans.py` 318 | Each moves when something outside the GUI needs what it holds. Not one item, and not a sweep **In progress, by hand, behind a net built first.** The tabs ask two questions between them -- *which scans does this result cover for this source* and *what distinct values are in these columns* -- and both are operations now: `export(method="scan_times")` and `export(method="distinct")`. **Done, as far as it honestly goes.** Twelve modules reached for the model; **two remain, and both are decided rather than owed** -- the scan and space-telescope editors turn what a user typed in a Qt widget into the model's own type, which is the dialog's job, and an operation taking a Qt datetime would be worse than the import. The eight visualization tabs asked three questions between them, all now operations: `distinct`, `scan_times`, and metadata read directly. The visualize dialog asked a fourth -- *which results can I offer* -- **by reading every one of them**: 142 ms and eleven frames held in memory to fill one combo box, which on a real project is the memory problem again. `available` counts from the parquet footers instead: **nothing resident**. An earlier attempt to do them all at once by regex deleted 851 lines while the suite stayed green, so `tests/test_gui_tabs.py` came first this time: it constructs each tab and *calls* the methods a first click reaches, verified by re-applying the exact damage and watching four tests fail. The batched edit is allowed again only with a guard that refuses any match containing a `def` |
