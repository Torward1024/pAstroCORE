...
| 2 | **M3** -- the request journal reaches the interface | A session's calculations are recorded and can be replayed against another project. `RequestJournal` exists in MSB and is exercised by one test here; nothing in the application writes to it | Days |
...
| 4 | **G3a** -- every result is written to disk twice | **Measured on one `source_visibility` request:** `times` stored 4 times, `telescope_positions` 2, `source_visibility` 2, and `Scan.to_dict` called 4 times. Two of those are structural -- `_get_cached_or_calculate` stores the frame with placeholder metadata and `_store_result` stores it again with the freshness stamp -- and since 0.7.0 a store reaches the disk, so it is two parquet writes per calculation. The fix is one store, and the care it needs is telling a fresh computation from a cached read: the guard that skips an unchanged write is what makes the cached path cheap. **Not to be rushed** -- this is the write-through path | Hours |
| 5 | **G2** then **G3** -- profile the interface, then act | A measured list of what is actually slow, with numbers; each finding fixed or recorded as not worth fixing. **The instrument is MSB's, not ours**: `RequestMetrics` counts and times every request per operation and `cache_statistics()` says what the caches are doing, so the measuring costs one interceptor rather than a harness. Two candidates already have a name: whether `fingerprint()` -- one serialisation per freshness check -- shows up at all, and whether `revision` would answer the same question for less | Days |
