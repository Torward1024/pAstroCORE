...
| 1 | **M3** -- the request journal reaches the interface | A session's calculations are recorded and can be replayed against another project. **Backend done:** `compute(method="history")`, `export(method="journal")` and `compute(method="replay")`, the last resolving each step's object by name -- which is possible at all because MSB 1.6.0 stopped holding what it recorded. What is left is the window: a panel that shows the session, saves it, and replays one | Hours |
