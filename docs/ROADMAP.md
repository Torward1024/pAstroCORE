...
| A3 | **The rest of the interface, tab by tab** | Counting lines in the hand-written GUI files that match no Qt name gives **5 861 of 7 772, about 75%** -- an *upper bound* on the gap rather than the amount of extractable logic, since much of it is glue that only means anything beside a widget. The one file read closely rather than counted was 252 of 312. Largest: `p_dialog_generate_observations.py` 361, `p_tab_telescopes.py` 344, `p_tab_scans.py` 318 | Each moves when something outside the GUI needs what it holds. Not one item, and not a sweep **In progress, and the first batch found the shape.** The ten visualization tabs were each asking the same question -- *which scans does this result cover for this source, and when does each start* -- with their own filter, group-by and MJD conversion, which is why each needed polars and astropy. One operation, `export(method="scan_times")`, answers it for all of them. Four tabs are off the debt list: **twelve to seven**. The remaining four ask different questions and get looked at individually rather than pattern-matched |
