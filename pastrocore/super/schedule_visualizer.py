...
class ScheduleVisualizer(Super):
    ...
    def _visualize(self, obj: Union[ScheduleProject, Observation, Telescope, SpaceTelescope, Telescopes, Source, Sources, Scan, Scans, IF, Frequencies], 
        ...
        Returns:
            Dict containing the visualization result data or error message.
        """
        ...
        if not plot_type:
            ...
            return self._build_response(obj, False, None, None, "plot_type required")

        # Always create a new figure to ensure consistency
        ...
        try:
            ...
            visualizer_result = visualizer(obj, attributes)
            ...
            # For single observations, add figure to result data if requested
            if isinstance(obj, Observation):
                if return_figure:
                    visualizer_result["figure"] = fig
                return self._build_response(obj, True, visualizer.__name__, visualizer_result)
            else:
                # For ScheduleProject or other objects, return the visualizer result as is
                if return_figure:
                    visualizer_result["figure"] = fig
                if not return_figure and not show:
                    plt.close(fig)
                return self._build_response(obj, True, visualizer.__name__, visualizer_result)
    ...
    def _visualize_project_or_observation(self, obj: Union[ScheduleProject, Observation], attributes: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if isinstance(obj, ScheduleProject):
            ...
            with ThreadPoolExecutor() as executor:
                ...
                results = {code: future.result()["result"] for future, code in futures.items() if future.result()["status"]}
            return results
    ...
    def _plot_time_on_source(self, obj: Observation, attributes: Dict[str, Any]) -> Dict[str, Any]:
        ...
        fig = plt.gcf()
        ax = fig.add_subplot(111, figsize=(12, len(telescopes)))
    ...
    def _plot_baseline_projections(self, obj: Observation, attributes: Dict[str, Any]) -> Dict[str, Any]:
        ...
        plt.title(f"Baseline Projections at {frequency/1e6:.1f} MHz")
        plt.legend()
        plt.grid(True)
    ...
    def _visualize_sources(self, obj: Union[Source, Sources], attributes: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if plot_type == "sky_position":
            ...
            plt.xlabel("Relative Right Ascension, (deg)")
