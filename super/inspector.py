...
from base.telescopes import Telescope, SpaceTelescope, Telescopes  # Добавляем импорт
...
class Inspector(ABC):
    ...
    def inspect(self, obj: Any, attributes: Dict[str, Any]) -> Dict[str, Any]:
        ...
        Args:
            obj: The object to inspect (e.g., IF, Frequencies, Source, Sources, Telescope, SpaceTelescope, Telescopes).
            attributes: Dictionary where keys are getter names and values are their arguments (or None if no args).
                       Example: {"get_name": None}
                       For nested inspection: {"telescope_index": 0, "get_code": None}
                       For SpaceTelescope: {"get_state_vector": {"dt": datetime(2025, 3, 26)}}
